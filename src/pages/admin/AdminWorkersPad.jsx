...
export default function AdminWorkersPad() {
  ...
  useEffect(() => {
    fetchNotes();
    const sub = supabase.channel('notes').on('postgres_changes', { event: '*', schema: 'public', table: 'admin_notes' }, fetchNotes).subscribe();
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-[80vh]">
      <div className="md:col-span-1 bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col">
        ...
        <textarea 
          className="flex-1 w-full p-3 border rounded-lg resize-none focus:outline-none focus:border-indigo-500" 
          placeholder="Type update here..."
          value={newNote}
          onChange={(e) => setNewNote(e.target.value)}
        ></textarea>
        <button onClick={addNote} className="mt-4 w-full py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">Post Note</button>
      </div>
        ...
        {notes.map(note => (
          <div key={note.id} className="bg-yellow-50 p-4 rounded-lg border border-yellow-200 shadow-sm relative group">
            ...
            <div className="flex justify-between items-center mt-3 text-xs text-gray-400">
              ...
              <button onClick={() => deleteNote(note.id)} className="text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition">Delete</button>
            </div>
          </div>
        ))}
      </div>
